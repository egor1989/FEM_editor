...
public class Drawing {
		...
		while (i < shapes.size()) {
			...
			if (common != null) {
				...
				if (shape.getType() >= s.getType())
					s1 = new BRepAlgoAPI_Cut(s.shape, shape.shape).shape();
				else s1 = s.shape;
				TopoDS_Shape s2 = common;
				TopoDS_Shape s3 = null;
				if (s.getType() >= shape.getType())
					s3 = new BRepAlgoAPI_Cut(shape.shape, s.shape).shape();
				else s3 = shape.shape;
				boolean s1empty = Shape.isEmpty(s1), 
				        s2empty = Shape.isEmpty(s2),
				        s3empty = Shape.isEmpty(s3);
	...
	public void explode(Shape s) {
		...
		if (s.type == Shape.COMPOUND) {
			shapes.remove(s);
			for (explorer.init(s.shape, TopAbs_ShapeEnum.SOLID); explorer.more(); explorer.next())
			{						
			...
			for (explorer.init(s.shape, TopAbs_ShapeEnum.FACE, TopAbs_ShapeEnum.SOLID); explorer.more(); explorer.next())
			{						
				TopoDS_Shape sh = explorer.current();
			...
			for (explorer.init(s.shape, TopAbs_ShapeEnum.EDGE, TopAbs_ShapeEnum.FACE); explorer.more(); explorer.next())
			{						
				TopoDS_Shape sh = explorer.current();
		...
		if (s.type == Shape.SOLID) {
			shapes.remove(s);
			for (explorer.init(s.shape, TopAbs_ShapeEnum.FACE); explorer.more(); explorer.next())
			{						
				TopoDS_Shape sh = explorer.current();
				if (!(sh instanceof TopoDS_Face)) continue; // should not happen!
				TopoDS_Face face = (TopoDS_Face)sh;
			...
			for (explorer.init(s.shape, TopAbs_ShapeEnum.EDGE); explorer.more(); explorer.next())
			{						
				TopoDS_Shape sh = explorer.current();
				if (!(sh instanceof TopoDS_Edge)) continue; // should not happen!
				TopoDS_Edge edge = (TopoDS_Edge)sh;
	...
	private Shape makeFace() {
		shapes.remove(lastEdge);
		TopoDS_Face face=(TopoDS_Face) new BRepBuilderAPI_MakeFace((TopoDS_Wire)lastEdge.shape).shape();
...
//	}
	...
	public Shape fillet(Shape s, double radius) {
		...
		TopExp_Explorer explorer = new TopExp_Explorer();
		for (explorer.init(s.shape, TopAbs_ShapeEnum.SOLID); explorer.more(); explorer.next()) {
			BRepFilletAPI_MakeFillet fillet = new BRepFilletAPI_MakeFillet(s.shape);
...
//				}				
	...
	public Shape copy(Shape s, double dx, double dy, double dz) {		
		...
		BRepBuilderAPI_Transform transform = new BRepBuilderAPI_Transform(s.shape,trsf, true);
	...
	public Shape[] copy(Shape[] shapes, double dx, double dy, double dz) {		
		...
		for (int i = 0; i < shapes.length; i++) {
			Shape s = shapes[i];
			transform.perform(s.shape, true);
			TopoDS_Shape newShape=transform.shape();
			Shape shape = new Shape(s.getType(), newShape, s.getVisualSettings());		
			add(shape);
			shapes1[i] = shape;
	...
	public Shape common(Shape s1, Shape s2) {
		shapes.remove(s1);
		shapes.remove(s2);
		TopoDS_Shape s = new BRepAlgoAPI_Common(s1.shape, s2.shape).shape();
	...
	public Shape cut(Shape s1, Shape s2) {
		...
		shapes.remove(s1);
		shapes.remove(s2);
		TopoDS_Shape s = new BRepAlgoAPI_Cut(s1.shape, s2.shape).shape();
		if (Shape.isEmpty(s)) return null;
	...
	public Shape fuse(Shape s1, Shape s2) {
		...
		shapes.remove(s1);
		shapes.remove(s2);
		TopoDS_Shape s = new BRepAlgoAPI_Fuse(s1.shape, s2.shape).shape();
		if (Shape.isEmpty(s)) return null;
		Shape shape = new Shape(s1.getType(), s, s1.getVisualSettings());		
		shapes.add(shape);				
		return shape;
